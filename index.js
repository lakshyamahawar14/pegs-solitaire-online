...
const _username = "test4";
const _score = 40;
var _isExist = false;


// db.run(`CREATE TABLE users (username TEXT, score INT)`);
db.get(`SELECT username FROM users WHERE username = "${_username}"`, (err, row) => {
    if (err) {
    ...
    if (row !== undefined) {
        ...
        stmt.finalize();
        db.each(`SELECT username, score FROM users ORDER BY score DESC`, (err, row) => {
            if (err) {
                return console.error(err.message);
            }
